...
class ApiRequest:
    ...
    def put(
        ...
        while retry > 0:
            try:
                # print(kwargs)
                if stream:
                    return self.client.stream("PUT", url, data=data, json=json, **kwargs)
                else:
    ...
    def get_characters(self):
        ...
        response = self.get(f"/character/characters", params=params)
        return self._get_response_value(response, as_json=True, value_func=lambda r: r.get("data", []))

    def create_character(self, name: str, description: str, definition: str):
        data = {
            "name": name,
            "description": description,
            "definition": definition
        }
        response = self.post(f"/character/characters", json=data)
        return self._get_response_value(response, as_json=True)

    def del_character(self, cid: int):
        params = [("ids", cid)]
        response = self.delete(f"/character/characters", params=params)
        return self._get_response_value(response, as_json=True)

    def update_character(self, cid: int, description: str, definition: str):
        data = {
            "description": description,
            "definition": definition
        }
        response = self.put(f"/character/characters/{cid}", json=data)
        return self._get_response_value(response, as_json=True)
    ...
    def chat(self, cid: int, text: str, history):
        data = {
            "text": text,
            "history": history
        }
        response = self.post(f"/character/chat/{cid}", json=data)
        return self._get_response_value(response, as_json=True, value_func=lambda r: r.get("data", ""))
...
def check_success_msg(data: Union[str, dict, list], key: str = "message") -> str:
    """
    return error message if error occured when requests API
    """
    if (isinstance(data, dict)
